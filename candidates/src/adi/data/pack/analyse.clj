(ns adi.data.pack.analyse
  (:require [hara.common :refer [error suppress assoc-if assoc-in-if assoc-nil hash-map?
                                 hash-set? long? keyword-join keyword-ns]]
            [adi.schema :as schema]
            [adi.schema.types :refer [db-id?]]
            [adi.common :refer [iid]]
            [adi.data.coerce :as ac]
     ...
     (if-let [[k v] (first tdata)]
       (let [subsch (get-in psch (conj nsv k))]
         (cond (hash-map? subsch)
               ...
               (vector? subsch)
               (recur (next tdata) psch nsv tsch fns
                      (assoc-if output (keyword-join (conj nsv k))
...
(defn analyse-nss [output]
 (let [pnss (or (get-in output [:# :nss]) #{})
       ...
       nss  (disj (set (map keyword-ns ks)) nil)]
   (assoc-in output [:# :nss] (clojure.set/union pnss nss))))
...
(defn wrap-nss [f]
 (fn [tdata psch nsv tsch fns]
   (let [output (f tdata psch nsv tsch fns)
     ...
     (if (empty? nsv) output
         (update-in output [:# :nss] #(clojure.set/union % (set nsv)))))))
