(ns adi.model
  (:require [hara.common :refer [error hash-map? merge-nested
                                 keyword-split keyword-join]]))
         ...
         (let [[attr] subsch]
           (if (= :ref (:type attr))
             (let [nnsv (keyword-split (-> attr :ref :ns))]
               (assoc output (keyword-join (conj nsv k))
 ...
 (if-let [subsch (get sch k)]
   (let [[attr] subsch]
     (cond (and (= :ref (:type attr))
           ...
           :else
           (assoc output (keyword-join (conj nsv k)) v)))
