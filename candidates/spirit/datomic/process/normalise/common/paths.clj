(ns spirit.process.normalise.common.paths
...
(defn wrap-plus
  ...
  (fn [tdata tsch nsv interim fns spirit]
    (let [output (f (dissoc tdata :+) tsch nsv interim fns spirit)
          pinterim  (normalise/submaps interim normalise/tree-directives :+)]
      (if-let [tplus (:+ tdata)]
        (let [pinterim (update-in pinterim [:key-path] conj :+)]
          (assoc output :+
                 ((:normalise fns) tplus (-> spirit :schema :tree) [] pinterim fns spirit)))
        output))))
...
(defn wrap-ref-path
  ...
  [f]
  (fn [tdata tsch nsv interim fns spirit]
    (f tdata tsch nsv (update-in interim [:ref-path] (fnil #(conj % tdata) [])) fns spirit)))
...
(defn wrap-key-path
  ...
  [f]
  (fn [tdata tsch nsv interim fns spirit]
    (f tdata tsch nsv (update-in interim [:key-path] (fnil #(conj % (last nsv)) [])) fns spirit)))