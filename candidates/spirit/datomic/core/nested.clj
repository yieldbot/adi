...
(defn update-in! [spirit data path update opts]
  (assert (even? (count path)) "The path must have a even number of items.")
  (let [spirit (prepare/prepare spirit opts data)
        ids (select/select spirit data {:options {:raw false}
                                     :return :ids})
        spath (partition 2 path)
        svec  (search-path-analysis spath (-> spirit :schema :tree))
        ndata-fn (build-search-term-fn svec)
        last-ns (:ns (last svec))
        update (if last-ns {last-ns update} update)
        output  (mapcat
                 #(transaction/update! (dissoc spirit :pipeline) (ndata-fn %) update
                                       {:options {:raw true
                                                  :ban-body-ids false
                                                  :ban-ids false
    ...
    (if (or (-> spirit :transact (= :datomic))
            (-> spirit :options :raw))
      transact
      (select/select (assoc spirit :db (:db-after transact))
                     (set sids)
...
(defn delete-in! [spirit data path opts]
  (assert (even? (count path)) "The path must have a even number of items.")
  (let [spirit (prepare/prepare spirit opts data)
        ids (select/select spirit data {:options {:raw false}
                                     :return :ids} )
        ...
        ndata-fn (build-search-term-fn svec)
        output  (mapcat
                 #(transaction/delete! (dissoc spirit :pipeline) (ndata-fn %)
                                       {:options {:raw true
                                                  :ban-body-ids false
                                                  :ban-ids false
                                                  :ban-top-id false}}) ids)
    ...
    (if (or (-> spirit :transact (= :datomic))
            (-> spirit :options :raw))
      transact
      (select/select (assoc spirit :db (:db-before transact))
                     (set sids)
                     (merge opts {:options {:spirit false
                                            :ban-ids false
                                            :ban-top-id false
...
(defn retract-in! [spirit data path retracts opts]
  (assert (even? (count path)) "The path must have a even number of items.")
  (let [spirit (prepare/prepare spirit opts data)
        ids (select/select spirit data {:options {:first false
                                               :raw false}
                                     :return :ids})
        spath (partition 2 path)
        svec  (search-path-analysis spath (-> spirit :schema :tree))
        ...
        output  (mapcat
                 (fn [id] (retract/retract! (dissoc spirit :pipeline)
                                           (ndata-fn id)
                                           nretracts
                                           {:options {:raw true
                                                      :ban-body-ids false
        ...
        transact (-> spirit
                     ...
                     (assoc-in [:process :emitted] output)
                     (transaction/transact-fn))]
    (if (or (-> spirit :options :raw)
            (-> spirit :transact (= :datomic)))
      transact
      (select/select (assoc spirit :db (:db-after transact))
                     (set sids)
                     (merge opts {:options {:spirit false
