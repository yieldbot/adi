(ns spirit.core.datomic.process.emit.query
  (:require [hara.common
             [error :refer [error]]
             [checks :refer [hash-map?]]]
            [spirit.core.datomic.data :refer [isym]]))
...
(defn query-data [chdata datasource]
...
(defn query-init [chdata datasource]
  (cond
   (nil? (seq chdata)) []
   :else
