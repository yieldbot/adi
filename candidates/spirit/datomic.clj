(ns spirit.core
  (:require [hara.namespace.import :as ns]
            ...
            [spirit.data.common :as data]))

(ns/import spirit.core.connection [connect! disconnect!]
           spirit.core.helpers    [transactions transaction-time schema-properties]
           spirit.schema          [schema]
           spirit.data.common     [iid])
...
(defn create-data-form [form spirit]
  (let [[f & args] form]
    (if (transaction-ops (resolve f))
      (concat (list f spirit) args (list :raw))
      (throw (AssertionError. (str "Only " transaction-ops " allowed."))))))

(defmacro sync-> [spirit args? & trns]
  (let [[opts trns] (if (vector? args?)
                      [(args->opts args?) trns] [{} (cons args? trns)])
        spiritsym (gensym)
        forms (filter identity (map #(create-data-form % spiritsym) trns))]
    `(let [~spiritsym  (prepare/prepare ~spirit (assoc ~opts :transact :datomic) nil)]
       (transact! ~spiritsym (concat ~@forms)))))
