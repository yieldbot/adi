(ns spirit.data.pipeline.fill-assoc-test
  (:use hara.test)
  (:require [spirit.data.pipeline :as pipeline]
            [spirit.data.pipeline.fill-assoc :as fill]
...
(def ^:dynamic *wrappers*
  {:normalise        [db/wrap-db pipeline/wrap-plus fill/wrap-model-fill-assoc]
   :normalise-nil    [ignore/wrap-nil-model-ignore]
   :normalise-branch [pipeline/wrap-key-path]
   :normalise-attr   [pipeline/wrap-key-path]})
   
^{:refer spirit.data.pipeline.fill-assoc/wrap-model-fill-assoc :added "0.3"}
