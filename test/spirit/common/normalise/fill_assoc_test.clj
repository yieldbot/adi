(ns spirit.common.normalise.fill-assoc-test
  (:use hara.test)
  (:require [spirit.common.normalise :as normalise]
            [spirit.common.normalise.fill-assoc :as fill]
...
(def ^:dynamic *wrappers*
  {:normalise        [db/wrap-db normalise/wrap-plus fill/wrap-model-fill-assoc]
   :normalise-nil    [ignore/wrap-nil-model-ignore]
   :normalise-branch [normalise/wrap-key-path]
   :normalise-attr   [normalise/wrap-key-path]})
   
^{:refer spirit.common.normalise.fill-assoc/wrap-model-fill-assoc :added "0.3"}
