(ns spirit.common.normalise.base.alias-test
  (:use hara.test)
  (:require [spirit.common.normalise :as normalise]
            [spirit.common.normalise.base.alias :refer :all]
            [spirit.datomic.process.normalise.db :as db]
            [spirit.datomic.process.normalise.set :as set]
            [spirit.common.schema :as schema]
            [data.family :as family]))

(def ^:dynamic *wrappers*
  {:normalise [db/wrap-db normalise/wrap-plus wrap-alias]
   :normalise-attr [set/wrap-attr-set]
   :normalise-branch [wrap-alias]})

^{:refer spirit.common.normalise.base.alias/wrap-alias :added "0.3"}
