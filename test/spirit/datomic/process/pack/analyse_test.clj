...
(fact "turns a nested-tree map into reference maps"
  ...
  => (throws-info {:id-banned true :id 2})
  ...
  (analyse-raw {:account {:user "Chris"
           ...
           {:schema (schema/schema examples/account-orders-items-image)
            :options {:ban-body-ids true}})
  => (throws-info {:body-id-banned true :id 3})

  (analyse-raw {:account {:user "Chris"
                      :orders #{100034}}}
           {:schema (schema/schema examples/account-orders-items-image)
            :options {:ban-body-ids true}})
  => (throws-info {:body-id-banned true :id 100034})
  ...
  => (throws-info {:expression-banned true :data '(= "Chris")})

  (analyse-raw {:account {:orders #{{:number '(= 5)}}}}
           {:schema (schema/schema examples/account-orders-items-image)
            :options {:ban-expressions true}})
  => (throws-info {:expression-banned true :data '(= 5)})
