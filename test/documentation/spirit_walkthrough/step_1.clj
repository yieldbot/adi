...
(facts
  ...
  (datomic/insert! ds {:account {:user "angela"   :password "hello1"}})

  (datomic/insert! ds [{:account {:user "billy"    :password "pass1"}}
  ...
  (datomic/select ds {:account {:password "pass1"}})
  ...
  (datomic/select ds {:account {:user "angela"}} :first true :ids true)
  ...
  (datomic/select ds {:account {:user "angela"}} :first :ids)
  => {:db {:id 17592186045418}, :account {:user "angela", :password "hello1", :credits 0}}
  ...
  (datomic/insert! ds {:account {:user "spirit" :password "hello"}})
  ...
  (datomic/insert! ds {:account {:user "spirit" :password "hello1" :type :vip}})
  ...
  (datomic/insert! {:account {:user "billy" :password "pass2"}})
  => throws
  ...
  (datomic/transactions ds :account/user)
  => [1001 1003]
  ...
  (datomic/transactions ds :account/user "angela")
  => [1001]

  "We can also see when the transaction has occured. This of course will be different for everyone:"

  (datomic/transaction-time ds 1001)  ;; -> #inst "2014-10-29T00:17:09.961-00:00"
  ...
  (datomic/select ds :account :at 1001)
  => #{{:account {:user "angela", :password "hello1", :credits 0}}}

  (datomic/select ds :account :at 1003)
  => #{{:account {:user "angela", :password "hello1", :credits 0}}
       {:account {:user "billy", :password "pass1", :credits 0}}
       {:account {:user "carl", :password "pass1", :credits 0}}}
