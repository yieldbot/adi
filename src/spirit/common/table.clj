   ...
   => {:account (prose/ascii
       ...
       :info (prose/ascii
              [\"| :id |   :name |\"
               ...
               \"|   2 | \\\"David\\\" |\"])}"
  {:added "0.5"}
  [data {:keys [file suffix headers levels sort-key id-key body?]
                          :as opts}]
  (if-not body? (fs/delete file))
  (if (zero? levels)
    (let [result (generate-single-table data opts)]
      (spit (str file "." suffix) result)
      result)
    (do (fs/create-directory file)
        (if-not body? (spit (str file "/" *properties-file*) (dissoc opts :file)))
        (reduce-kv (fn [out k v]
                     (let [sopts {:file (str file "/" (name k))
                                  :suffix   suffix
  ...
  "generates a single table
   ...
   => {:account {\"a@a.com\" {:id 1 :value \"a\"}
       ...
       :info {1 {:name \"Chris\"}
              ...
              3 {:name \"Cain\"}}}"
  {:added "0.5"}
  [{:keys [file suffix headers levels sort-key id-key body?]
    :as opts}]
  (cond (fs/file? file)
        (parse-single-table (slurp file) opts)

        :else
        (let [{:keys [suffix headers levels sort-key id-key body?]
               :as opts}
              (merge (if-not body? (try (read-string (slurp (str file "/" *properties-file*)))
                                        (catch Exception e)))
                     opts)
